...
export class Sole
{
     ...
     */
    isThereAnythingRelatedTo( query, searchData )
    {
        let ratedResults;
        ...
        if ( typeof searchData === 'undefined' || searchData.length < 1 )
            return false;
        ...
        if ( query.length >= defaults.minimumValueLength )
        {
            ...
            // console.log( 'querry is good,', query );
            ...
            ratedResults    = this.ratedResults = data.map( function( d, i )
            {
                ...
                // console.log( 'IN THERE RESULTS', search );
